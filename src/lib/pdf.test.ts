import { SAMPLE_BPA1_TEXT } from './tax'
import { createSampleBpa1Pdf, extractBpa1FromPdf, validatePdfFile } from './pdf'
...
describe('fictional BPA1 PDF boundary', () => {
  it('creates a local PDF containing the fictional fixture', () => {
    ...
    expect(new TextDecoder().decode(bytes)).toContain('LAPORAI BPA1 FIKTIF')
  })

  it('extracts the supplied generated PDF end to end', async () => {
    const file = new File([createSampleBpa1Pdf(SAMPLE_BPA1_TEXT)], 'LaporAI-BPA1-FIKTIF.pdf', { type: 'application/pdf' })
    const record = await extractBpa1FromPdf(file)
    expect(record.employer).toBe('PT NUSANTARA TEKNOLOGI FIKTIF')
